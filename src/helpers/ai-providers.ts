 ...
 * 
 * Shared between popup and content scripts.
 * Icons are defined as raw SVG strings for use in DOM (content script)
 * and as a reference for which icons to use.
 */
...
// AI Provider SVG icons (LobeHub-sourced, mono variants)
...
/**
 * Load AI providers from storage, merging with defaults
 */
export async function loadAIProviders(): Promise<AIProviderConfig[]> {
  return new Promise((resolve) => {
    chrome.storage.sync.get(['aiProviders'], (result) => {
      if (result.aiProviders && Array.isArray(result.aiProviders)) {
        // Settings saves only { id, enabled } - merge with defaults
        const savedProviders = result.aiProviders as Array<{ id: string; enabled: boolean }>;
        const merged = DEFAULT_AI_PROVIDERS.map(defaultProvider => {
          const saved = savedProviders.find(p => p.id === defaultProvider.id);
          return saved !== undefined
            ? { ...defaultProvider, enabled: saved.enabled }
            : defaultProvider;
        });
        resolve(merged);
      } else {
        resolve(DEFAULT_AI_PROVIDERS);
      }
    });
  });
}
 ...
 */
export async function getEnabledAIProviders(): Promise<AIProviderConfig[]> {
  const providers = await loadAIProviders();
  const enabled = providers.filter(p => p.enabled);
  console.log('[AIProviders] Loaded providers:', providers.map(p => `${p.id}:${p.enabled}`).join(', '));
  console.log('[AIProviders] Enabled providers:', enabled.map(p => p.id).join(', '));
  return enabled;
}
...
export function getProviderIcon(providerId: string): string {
  return AI_PROVIDER_ICONS[providerId] || AI_PROVIDER_ICONS.chatgpt;
}
